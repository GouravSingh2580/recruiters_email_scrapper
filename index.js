...
const driver = async () => {
    ...
    var arr = [];
    var results = [];
    log('Hello, please make sure that the input.csv file has been written with the list of companies you want');
    log('type y to continue');
    ans = query('');
    if (!ans) {
        log('program shutting down, please restart');
        return;
    }
    fs.createReadStream('input.csv')
        ...
        .on('end', async () => {
            ...
            log('CSV file successfully written, please check the output.csv file');
        });
};
